...
function buildDeterministicRecords(q: string, data: Record<string, any[]>) {
  ...
  (data.loans || []).forEach((r: any) => push(r.id, "loan", `View loan ${r.loan_reference || r.id}`, `${r.borrower_name || r.borrower_email || "Borrower"} • UGX ${Number(r.principal || r.amount || 0).toLocaleString()} • ${r.status || "pending"}`, "/finance", 92, { reference: r.loan_reference }));
  (data.loan_repayments || []).forEach((r: any) => push(r.id, "loan", `View loan repayment ${r.reference || r.id}`, `UGX ${Number(r.amount || 0).toLocaleString()} • ${r.status || "recorded"}`, "/finance", 89, { reference: r.reference, loan_id: r.loan_id }));
  (data.loan_appeals || []).forEach((r: any) => push(r.id, "loan", `View loan appeal ${r.id}`, `${r.appellant_name || "Borrower"} • ${r.status || "pending"} • ${r.reason || ""}`, "/finance", 87, { loan_id: r.loan_id }));
  ...
  (data.investments || []).forEach((r: any) => push(r.id, "investment", `View investment ${r.reference || r.id}`, `${r.investor_name || "Investor"} • UGX ${Number(r.amount || 0).toLocaleString()} • ${r.status || "active"}`, "/finance", 87, { reference: r.reference }));
