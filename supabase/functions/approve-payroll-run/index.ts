...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      !["Super Admin", "Administrator"].includes(caller.role)
    ) {
      return new Response(
        JSON.stringify({
          ok: false,
    ...
    if (run.status === "disbursed") {
      return new Response(
        JSON.stringify({ ok: false, error: "Already disbursed" }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const invokeResp = await fetch(
      `${supabaseUrl}/functions/v1/process-auto-salaries`,
      {
        method: "POST",
        headers: {
    ...
    if (!invokeResp.ok) {
      throw new Error(
        `process-auto-salaries failed (${invokeResp.status}): ${JSON.stringify(result)}`,
      );
    }
    ...
    return new Response(
      JSON.stringify({ ok: true, status: "disbursed", result }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      },
    );
