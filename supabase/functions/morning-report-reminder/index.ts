...
serve(async (req) => {
  ...
  try {
    ...
    const reminderSettings = settings?.setting_value || { morning_report_reminder: true };
    
    if (!reminderSettings.morning_report_reminder) {
      console.log('Morning report reminder is disabled in system settings');
      return new Response(
        JSON.stringify({ message: 'Morning report reminder is disabled', enabled: false }),
