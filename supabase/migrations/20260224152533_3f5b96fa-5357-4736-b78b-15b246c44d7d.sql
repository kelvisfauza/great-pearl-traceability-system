...
-- 1. Remove the 30-minute cooldown from award_activity_reward
...
BEGIN
  ...
  -- NO MORE 30-minute cooldown check
...
-- 2. Create fraud locks table
CREATE TABLE IF NOT EXISTS public.user_fraud_locks (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  user_email TEXT NOT NULL,
  user_name TEXT,
  unlock_code TEXT NOT NULL,
  reason TEXT DEFAULT 'Suspected rapid page browsing for rewards',
...
ALTER TABLE public.user_fraud_locks ENABLE ROW LEVEL SECURITY;

-- Users can read their own lock status
CREATE POLICY "Users can view own fraud locks"
  ON public.user_fraud_locks
  FOR SELECT
  USING (user_id = auth.uid());

-- Allow insert from authenticated users (system inserts on behalf)
CREATE POLICY "Authenticated users can insert fraud locks"
  ON public.user_fraud_locks
  FOR INSERT
  TO authenticated
  WITH CHECK (true);

-- Allow update for unlocking (authenticated users can update their own)
CREATE POLICY "Users can update own fraud locks"
  ON public.user_fraud_locks
  FOR UPDATE
  USING (user_id = auth.uid());
