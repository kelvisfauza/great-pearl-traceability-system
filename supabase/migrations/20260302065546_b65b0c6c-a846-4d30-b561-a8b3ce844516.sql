...
-- Fix: Remove 'approved' from pending statuses in get_user_balance_safe
-- because the trigger already created a negative ledger entry when status became 'approved'
-- This was causing DOUBLE DEDUCTION of approved withdrawal amounts
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    LEFT JOIN (
      ...
      FROM withdrawal_requests wr
      WHERE wr.status IN ('pending', 'processing', 'pending_approval', 'pending_admin_2', 'pending_admin_3', 'pending_finance')
        AND wr.user_id = public.get_unified_user_id(user_email)
      GROUP BY wr.user_id
