...
BEGIN
  IF NEW.status = 'approved' AND OLD.status != 'approved' THEN
    ...
    IF NEW.request_type = 'withdrawal' THEN
      ...
      -- Store balance info for audit but allow approval to proceed
      -- The user had sufficient funds at submission time; balance may have changed due to loan recoveries
      NEW.wallet_balance_at_approval := current_user_balance;
      NEW.wallet_balance_verified := (current_user_balance >= NEW.amount);
      ...
      INSERT INTO public.ledger_entries (
        ...
        json_build_object(
          ...
          'approved_at', now(),
          'balance_at_approval', current_user_balance,
          'balance_sufficient', (current_user_balance >= NEW.amount)
        ),
        now()
      );
