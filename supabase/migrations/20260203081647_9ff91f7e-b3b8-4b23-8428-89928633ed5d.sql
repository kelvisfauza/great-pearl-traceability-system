-- Drop and recreate the function to also migrate quality_assessments and payment_records directly
...
BEGIN
  ...
  LOOP
    ...
    -- Update coffee_records and related tables
  ...
  -- Process quality_assessments with old format that weren't linked to coffee_records
  FOR record_row IN
    SELECT id, batch_number, COALESCE(date_assessed, created_at::date) as record_date
  ...
  LOOP
    ...
    UPDATE quality_assessments SET batch_number = new_batch WHERE id = record_row.id;
    UPDATE payment_records SET batch_number = new_batch WHERE batch_number = record_row.batch_number;
    
    migrated_count := migrated_count + 1;
  END LOOP;

  -- Process payment_records with old format that weren't linked
  FOR record_row IN
    SELECT id, batch_number, COALESCE(date, created_at::date) as record_date
  ...
  LOOP
    ...
    UPDATE payment_records SET batch_number = new_batch WHERE id = record_row.id;
    migrated_count := migrated_count + 1;
  END LOOP;

  -- Process store_records with old format
  FOR record_row IN
  ...
  RETURN jsonb_build_object('migrated', migrated_count, 'failed', 0);
END;
$$;