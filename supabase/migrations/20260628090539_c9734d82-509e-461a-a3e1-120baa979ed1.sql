-- Helper to get spendable balance (effective minus partial lock like attendance 35%)
CREATE OR REPLACE FUNCTION public.get_spendable_wallet_balance(p_user_id text)
RETURNS numeric
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  ...
  v_email text;
BEGIN
  v_eff := COALESCE(public.get_effective_wallet_balance(p_user_id), 0);

  -- Resolve email for this unified user id and look up employees.wallet_locked_amount
  BEGIN
    SELECT email INTO v_email FROM public.employees
      WHERE auth_user_id::text = p_user_id OR email = p_user_id
      LIMIT 1;
  EXCEPTION WHEN OTHERS THEN v_email := NULL; END;

  IF v_email IS NOT NULL THEN
    SELECT COALESCE(wallet_locked_amount, 0) INTO v_locked
    FROM public.employees WHERE email = v_email LIMIT 1;
  END IF;

  -- Only subtract lock when wallet is positive; never push into negative due to lock.
  IF v_eff > 0 AND v_locked > 0 THEN
    RETURN GREATEST(v_eff - v_locked, 0);
  END IF;
  RETURN v_eff;
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_spendable_wallet_balance(text) TO authenticated, service_role;

-- Update transfer function to enforce the lock
...
BEGIN
  ...
  -- Spendable = effective minus locked amount (e.g., 35% attendance lock)
