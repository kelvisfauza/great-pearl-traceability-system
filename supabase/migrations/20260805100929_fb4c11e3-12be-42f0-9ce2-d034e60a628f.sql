...
BEGIN
  ...
  -- Hard ceiling per single action so one payment cannot swallow the day's budget
  per_action_cap := CASE
    WHEN is_finance_ops AND activity_name IN ('transaction', 'report_generation', 'form_submission', 'task_completion') THEN 1200
    WHEN activity_name IN ('voice_call', 'group_meeting', 'departmental_meeting') THEN 800
    ELSE 400
  END;

  IF is_finance_ops AND activity_name IN ('transaction', 'report_generation') THEN
    actual_reward := GREATEST(actual_reward, 300);
  END IF;
