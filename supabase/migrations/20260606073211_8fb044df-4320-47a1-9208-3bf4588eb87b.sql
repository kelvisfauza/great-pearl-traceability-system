...
CREATE OR REPLACE FUNCTION public.consume_spendable(p_user_id uuid, p_amount numeric, p_source text, p_reference text DEFAULT NULL::text, p_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_rate_bps integer := 50; -- default 0.5%
BEGIN
  ...
  IF FOUND AND NOT v_acc.frozen THEN
    ...
    v_rate_bps := COALESCE(v_acc.interest_rate_bps, 50);
  END IF;
  ...
  IF v_od_part > 0 AND v_acc.id IS NOT NULL THEN
    -- Upfront interest charged at the moment user accepts the overdraft
    v_interest := ROUND(v_od_part * v_rate_bps / 10000.0, 2);
    ...
    v_new_out := v_acc.outstanding_balance + v_od_part + v_interest;
    ...
    VALUES
      (v_acc.id, p_user_id, 'draw', v_od_part, v_acc.outstanding_balance + v_od_part, v_od_ledger_id, v_ref,
    ...
    IF v_interest > 0 THEN
      INSERT INTO public.overdraft_transactions
        (account_id, user_id, transaction_type, amount, balance_after, ledger_entry_id, reference, metadata)
      VALUES
        (v_acc.id, p_user_id, 'interest', v_interest, v_new_out, NULL, v_ref || '-INT',
         jsonb_build_object('source', p_source, 'rate_bps', v_rate_bps, 'draw_amount', v_od_part,
                            'description', 'Upfront interest on overdraft draw (' || (v_rate_bps/100.0) || '%)'));
    END IF;
  END IF;
