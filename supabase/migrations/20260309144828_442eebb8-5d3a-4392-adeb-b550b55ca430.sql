...
BEGIN
  ...
  LEFT JOIN (
    ...
    WHERE le.entry_type IN ('LOYALTY_REWARD', 'BONUS', 'DEPOSIT', 'WITHDRAWAL', 'ADJUSTMENT', 'DAILY_SALARY')
    GROUP BY le.user_id
  ...
  IF NOT FOUND THEN
    ...
    FROM (
      ...
      WHERE le.user_id = public.get_unified_user_id(user_email)
        AND le.entry_type IN ('LOYALTY_REWARD', 'BONUS', 'DEPOSIT', 'WITHDRAWAL', 'ADJUSTMENT', 'DAILY_SALARY')
      GROUP BY le.user_id
    ) ledger
    LEFT JOIN (
      SELECT 
