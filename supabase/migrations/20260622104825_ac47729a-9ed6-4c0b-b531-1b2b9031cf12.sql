...
-- 1) Drop the legacy 3-arg overload that lacks overdraft logic.
DROP FUNCTION IF EXISTS public.transfer_wallet_funds_secure(text, numeric, text);

-- 2) Rewrite the OD-aware version to use get_effective_wallet_balance
--    (matches what the UI shows; excludes non-spendable ledger entries).
...
BEGIN
  ...
  -- Use the effective wallet balance (same source of truth as the UI)
  ...
  RETURN json_build_object(
    ...
    'overdraft_fee', v_od_fee,
    'sender_available_balance', GREATEST(0, v_sender_balance - p_amount),
...
GRANT EXECUTE ON FUNCTION public.transfer_wallet_funds_secure(text, numeric, text, boolean) TO authenticated;

-- 3) Backfill the missed OD draw for Fauza's transfer that slipped through
--    via the legacy overload. Effective wallet went to -144,095; we charge
--    the shortfall + 2.75% fee against her overdraft (limit 150k, room 150k).
DO $$
DECLARE
  v_user_id text;
  v_acct_id uuid;
  v_outstanding numeric;
  v_shortfall numeric := 144095;
  v_fee numeric;
  v_new_outstanding numeric;
  v_led_id uuid;
BEGIN
  SELECT COALESCE(auth_user_id::text, id::text) INTO v_user_id
  FROM public.employees WHERE email = 'fauzakusa@greatpearlcoffee.com' LIMIT 1;

  SELECT id, outstanding_balance INTO v_acct_id, v_outstanding
  FROM public.overdraft_accounts
  WHERE employee_email = 'fauzakusa@greatpearlcoffee.com' AND status = 'active' LIMIT 1;

  IF v_acct_id IS NULL OR v_user_id IS NULL THEN
    RAISE NOTICE 'Fauza OD account or user id not found; skipping backfill';
    RETURN;
  END IF;

  v_fee := round(v_shortfall * 0.0275);
  v_new_outstanding := v_outstanding + v_shortfall + v_fee;

  INSERT INTO public.ledger_entries (user_id, entry_type, amount, reference, source_category, metadata)
  VALUES (
    v_user_id, 'DEPOSIT', v_shortfall,
    'OD-BACKFILL-FAUZA-DENIS-' || to_char(now(),'YYYYMMDDHH24MISS'),
    'OVERDRAFT_DRAW',
    jsonb_build_object(
      'type','overdraft_draw',
      'overdraft_account_id', v_acct_id,
      'reason','Backfill: transfer to bwambale denis (SEND-1782124863831-8FJY) used wallet but legacy RPC skipped OD logging',
      'fee_amount', v_fee,
      'fee_rate', 0.0275,
      'bypass_treasury_check', true,
      'description','Overdraft draw backfill for transfer (UGX '||v_shortfall::text||' + UGX '||v_fee::text||' fee)'
    )
  ...
  INSERT INTO public.overdraft_transactions (account_id, user_id, transaction_type, amount, balance_after, ledger_entry_id, reference, metadata)
  VALUES (v_acct_id, v_user_id::uuid, 'draw', v_shortfall, v_new_outstanding, v_led_id,
          'OD-XFER-BACKFILL-'||extract(epoch from now())::bigint::text,
          jsonb_build_object('reason','Backfill transfer top-up','fee_amount', v_fee,'fee_rate',0.0275,'transfer_ref','SEND-1782124863831-8FJY'));

  INSERT INTO public.overdraft_transactions (account_id, user_id, transaction_type, amount, balance_after, reference, metadata)
  VALUES (v_acct_id, v_user_id::uuid, 'fee', v_fee, v_new_outstanding,
          'OD-XFER-FEE-BACKFILL-'||extract(epoch from now())::bigint::text,
          jsonb_build_object('fee_rate',0.0275,'draw_amount',v_shortfall,'note','2.75% access fee on backfilled transfer top-up','transfer_ref','SEND-1782124863831-8FJY'));
END $$;
