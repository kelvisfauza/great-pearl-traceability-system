...
BEGIN
  ...
  IF v_meta_type IN (
    ...
    'overdraft_interest','wallet_transfer_out','wallet_transfer_in',
    'wallet_transfer_reversal'
  ) THEN
    RETURN NEW;
  END IF;

  -- Internal synthetic accruals and pure ledger-only movements (no real cash) bypass float checks
  IF v_source IN (
    'OVERDRAFT_INTEREST','OVERDRAFT_FEE','LOAN_INTEREST','INTEREST_ACCRUAL',
