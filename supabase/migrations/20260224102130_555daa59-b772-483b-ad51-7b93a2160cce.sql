...
BEGIN
  ...
  -- Calculate remaining working days in month (exclude Sundays)
  ...
  -- Get total rewards already earned this month
  ...
  -- Calculate daily budget: spread remaining cap across remaining days
  daily_budget := remaining_cap / remaining_days;

  -- Get how much already earned today
  SELECT COALESCE(SUM(amount), 0), COUNT(*)
  INTO today_earned, today_actions
  FROM public.ledger_entries
  WHERE user_id = user_uuid::TEXT
    AND entry_type = 'LOYALTY_REWARD'
  ...
  IF today_budget <= 0 THEN
    RETURN json_build_object(
      'success', false,
      'message', 'Daily loyalty budget used up, keep going tomorrow!',
      'monthly_total', monthly_total,
      'reward_given', 0
    );
  END IF;

  -- Activity weight (higher weight = bigger share of daily budget)
  CASE activity_name
    WHEN 'data_entry' THEN base_weight := 0.4;
    WHEN 'form_submission' THEN base_weight := 0.8;
    WHEN 'report_generation' THEN base_weight := 1.2;
    WHEN 'task_completion' THEN base_weight := 0.8;
    WHEN 'document_upload' THEN base_weight := 0.6;
    WHEN 'transaction' THEN base_weight := 1.0;
    ELSE base_weight := 0.3;
  END CASE;

  -- Calculate reward: fraction of remaining daily budget based on weight
  -- Divide by expected actions per day (~15) and multiply by weight
  actual_reward := ROUND((today_budget / 15.0) * base_weight);

  -- Ensure minimum of 1 and maximum of remaining cap
  ...
  INSERT INTO public.ledger_entries (
    ...
    json_build_object(
      ...
      'today_earned', today_earned + actual_reward,
