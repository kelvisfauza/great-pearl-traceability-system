...
DECLARE
  ...
  meta_payload JSONB;
BEGIN
  -- Effective 1 August 2026: cap raised to 120k and per-activity rewards tripled.
  ...
  -- High-value finance operations (supplier payments, GRN payment receipts, payouts)
  is_finance_ops := ctx_blob ~ '(paying supplier|supplier payment|payment receipt|grn|finance|payout|disburse)';
  ...
  activity_daily_limit := CASE activity_name
    WHEN 'chat_message' THEN 20
    WHEN 'voice_call' THEN 5
    WHEN 'group_meeting' THEN 4
    WHEN 'departmental_meeting' THEN 3
    ELSE NULL
  END;

  IF activity_daily_limit IS NOT NULL THEN
  ...
  -- Finance operations may draw up to 3 days of budget in a single day
  today_budget := GREATEST((CASE WHEN is_finance_ops THEN daily_budget * 3 ELSE daily_budget END) - today_earned, 0);
  ...
  -- Finance payment processing / receipt printing carry a 4x weight boost
  ...
  IF is_finance_ops AND activity_name IN ('transaction', 'report_generation') THEN
    actual_reward := GREATEST(actual_reward, 500);
  END IF;
