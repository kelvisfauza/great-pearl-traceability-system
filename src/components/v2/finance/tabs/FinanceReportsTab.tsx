...
import { Loader2, FileText, TrendingDown, Wallet } from "lucide-react";
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [todayPayments, weekPayments, cashTx] = await Promise.all([
        supabase.from('supplier_payments').select('amount_paid_ugx, created_at')
      ...
      const todayTotal = todayPayments.data?.reduce((s, p) => s + (p.amount_paid_ugx || 0), 0) || 0;
      const weekTotal = weekPayments.data?.reduce((s, p) => s + (p.amount_paid_ugx || 0), 0) || 0;
