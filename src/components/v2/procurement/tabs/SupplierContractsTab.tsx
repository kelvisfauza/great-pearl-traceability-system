...
const SupplierContractsTab = () => {
  const { contracts, loading } = useSupplierContracts();
  ...
  const voided = contracts.filter(c => c.status === 'Voided');
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              ...
              {filtered.length === 0 && (
                <TableRow><TableCell colSpan={8} className="text-center py-6 text-muted-foreground">No supplier contracts found</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
