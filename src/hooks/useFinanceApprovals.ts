...
export const useFinanceApprovals = () => {
    ...
    try {
      ...
      try {
        ...
        if (advanceEmployee?.phone) {
          const message = `Dear ${advanceEmployee.name}, your salary advance of UGX ${details.advance_amount.toLocaleString()} has been APPROVED and DISBURSED. Minimum monthly payment: UGX ${details.minimum_payment.toLocaleString()}. This will be deducted from your future salary requests. Great Agro Coffee.`;
  ...
  const handleFinanceApproval = async (
    ...
    try {
      ...
      // POLICY: When paying My Expense requests (Personal Expense / Cash Requisition / Salary Request)
      // Finance must enter the reference number printed on the paper requisition,
      // and that reference must exist in expense_template_refs.
      const myExpenseTypes = ['Cash Requisition', 'Personal Expense', 'Salary Request'];
      if (approve && request && myExpenseTypes.includes(request.type)) {
        const refTrim = (documentRef || '').trim();
        if (!refTrim) {
          toast({
            title: 'Document Reference Required',
            description: `Enter the Ref printed on the ${request.type} paper before paying.`,
            variant: 'destructive',
          });
          return { needsRef: true } as any;
        }
        const { data: refRow } = await supabase
          .from('expense_template_refs' as any)
          .select('ref, used, employee_email, approval_type')
          .eq('ref', refTrim)
          .maybeSingle();
        if (!refRow) {
          toast({
            title: 'Reference Not Found',
            description: `Ref "${refTrim}" was never generated by the system. Cannot pay.`,
            variant: 'destructive',
          });
          return { invalidRef: true } as any;
        }
        // Mark ref as used and link to the request
        try {
          await supabase
            .from('expense_template_refs' as any)
            .update({
              ...
              used_by: employee?.name || employee?.email || 'Finance',
              request_id: requestId,
            })
            .eq('ref', refTrim);
        } catch (e) {
          console.warn('Could not mark ref used:', e);
        }
      }
