  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        withDateRange(supabase.from("sales_transactions").select("total_amount, weight, customer", { count: "exact" }), "date"),
        withDateRange(supabase.from("buyer_contracts").select("id, status, total_quantity, price_per_kg", { count: "exact" })),
        withDateRange(supabase.from("eudr_sales").select("id", { count: "exact" }), "sale_date"),
