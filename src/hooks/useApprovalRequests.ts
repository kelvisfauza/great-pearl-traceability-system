...
const sendExpenseApprovalNotification = async (request: ApprovalRequest) => {
  try {
    ...
    const message = `Dear ${userName}, your expense ${request.title} of UGX ${request.amount.toLocaleString()} has been approved and dispersed successfully. Great Agro Coffee.`;
  ...
  const updateRequestStatus = async (
    ...
    try {
      ...
      if (status === 'Rejected') {
        ...
        // ✅ NEW FLOW: Admin approves first, then goes to Finance portal
        if (approvalType === 'admin') {
          updateData.admin_approved = true;
          updateData.admin_approved_at = new Date().toISOString();
          updateData.admin_approved_by = approverName || 'Admin Team';
          updateData.status = 'Pending Finance'; // ✅ Admin approved, awaiting Finance portal
      ...
      try {
        ...
        if (updateData.status === 'Approved' && updateData.approval_stage === 'fully_approved') {
          ...
          // Send approval notification for ALL request types
          try {
